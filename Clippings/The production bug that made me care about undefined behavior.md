...
source: "https://gaultier.github.io/blog/the_production_bug_that_made_me_care_about_undefined_behavior.html"
author:
published:
created: 2025-12-31
description:
...
**Table of contents**
- [The bug report](https://gaultier.github.io/blog/#the-bug-report)
- [Investigating](https://gaultier.github.io/blog/#investigating)
- [Just enough rope to hang yourself](https://gaultier.github.io/blog/#just-enough-rope-to-hang-yourself)
- [Aftermath](https://gaultier.github.io/blog/#aftermath)
- [Static analysis to the rescue?](https://gaultier.github.io/blog/#static-analysis-to-the-rescue)
- [Runtime analysis to the rescue](https://gaultier.github.io/blog/#runtime-analysis-to-the-rescue)
- [The aftermath](https://gaultier.github.io/blog/#the-aftermath)
- [Some bonus C++ rules for you](https://gaultier.github.io/blog/#some-bonus-cplusplus-rules-for-you)
- [Conclusion](https://gaultier.github.io/blog/#conclusion)
- [Addendum](https://gaultier.github.io/blog/#addendum)
...
2. If `T` is an array, e.g. `std::string a[10];`, this is fine: each element is default-initialized. But note that some types do not have default initialization, such as `int`: `int a[10]` would leave each element uninitialized.
3. If `T` is a [POD](https://en.cppreference.com/w/cpp/named_req/PODType) (Plain Old Data, pre C++11. The wording in the standard changed with C++11 but the idea remains under the term Trivially Default Constructible) struct, e.g. `Foo foo;` no initialization is performed at all. This is akin to doing `int a;` and then reading `a`. This is obvious undefined behavior.
4. If `T` is a non-POD struct, e.g. `Bar bar;` the default constructor is called, and it is responsible for initializing all fields. It is easy to miss one, or even forget to implement a default constructor entirely, leading to undefined behavior.
...
```shell
Shell$ clang++ main.cpp -Weverything -std=c++11 -g -fsanitize=address,undefined -Wno-padded
$ ./a.out
...
```cpp
Cpp     unsigned char c;     // “c” has an indeterminate/erroneous value
...
A commenter posted this Forrest Gump gif that I had completely forgotten about, so thank you:)
  ...
  <video controls=""><source src="/blog/forrest_gump_c++.mp4" type="video/mp4"> <source src="/blog/forrest_gump_c++.webm" type="video/webm"> <source src="/blog/forrest_gump_c++.gif" type="image/gif"></video>
